...
class AdminController extends Controller
{
    ...
    {
        // Ambil semua permintaan yang ada, diurutkan dari yang terbaru
        $requests = PasswordRequest::orderBy('created_at', 'desc')->get();
        
        // Ambil detail pengguna (Guru/Siswa) untuk ditampilkan
        foreach ($requests as $request) {
            ...
            if ($request->role == 2) { 
                ...
                $user = Siswa::with('kelas')->firstWhere('email', $request->email);
                $info = ($user && $user->kelas) ? 'Siswa - Kelas ' . $user->kelas->nama_kelas : 'Siswa - Kelas N/A';
            }

            $request->user_detail = [
                'nama' => $user ? ($request->role == 2 ? $user->nama_guru : $user->nama_siswa) : 'User Not Found',
                'tipe' => $info,
            ];
        }
    ...
    // --- BARU: Logic AGREE (Ubah Password dan Hapus Request) ---
    public function agreePasswordRequest(PasswordRequest $passwordRequest)
    {
        $email = $passwordRequest->email;
        $newPassword = $passwordRequest->requested_password;

        if (empty($newPassword)) {
            // Ini seharusnya tidak terjadi jika validasi AuthController benar, tapi sebagai safety check
            $passwordRequest->delete();
            return redirect('/admin/password-requests')->with('pesan', "
                <script>
                    swal({
                        title: 'Error!',
                        text: 'Gagal menyetujui. Password permintaan kosong.',
                        type: 'error',
                        padding: '2em'
                    })
                </script>
        ...
        try {
            if ($passwordRequest->role == 2) { // Guru
                Guru::where('email', $email)->update(['password' => $hashedPassword]);
            } elseif ($passwordRequest->role == 3) { // Siswa
                Siswa::where('email', $email)->update(['password' => $hashedPassword]);
            }
            
            // Hapus permintaan setelah berhasil diproses
            $passwordRequest->delete(); 
            ...
            return redirect('/admin/password-requests')->with('pesan', "
                <script>
                    swal({
                        title: 'Berhasil!',
                        text: 'Password untuk $email berhasil diubah menjadi: $newPassword. Permintaan disetujui dan diselesaikan.',
                        type: 'success',
                        padding: '2em'
                    })
                </script>
            ");
             ...
             return redirect('/admin/password-requests')->with('pesan', "
                <script>
                    swal({
                        title: 'Error!',
                        text: 'Gagal memproses permintaan: " . str_replace('\'', '\`', $e->getMessage()) . "',
                        type: 'error',
                        padding: '2em'
                    })
                </script>
    ...
    // --- BARU: Logic DISAGREE (Hapus Request Tanpa Ubah Password) ---
    public function disagreePasswordRequest(PasswordRequest $passwordRequest)
    {
        $email = $passwordRequest->email;
        
        // Hapus permintaan tanpa mengubah password
        $passwordRequest->delete(); 
        ...
        return redirect('/admin/password-requests')->with('pesan', "
            <script>
                swal({
                    title: 'Berhasil!',
                    text: 'Permintaan ganti password untuk $email berhasil DITOLAK dan dihapus.',
                    type: 'info',
                    padding: '2em'
                })
            </script>
